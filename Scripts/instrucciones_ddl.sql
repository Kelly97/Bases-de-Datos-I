...
COMMENT ON COLUMN tbl_estado_notificacion.codgio_estado_notificacion IS
...
ALTER TABLE tbl_estado_notificacion ADD CONSTRAINT tbl_estado_notificacion_pk PRIMARY KEY ( codgio_estado_notificacion );
...
CREATE TABLE tbl_flips_x_usuario (
    codigo_noticia   INTEGER NOT NULL,
    codigo_usuario   INTEGER NOT NULL,
    codigo_revista   INTEGER NOT NULL,
    fecha            DATE NOT NULL
...
COMMENT ON COLUMN tbl_flips_x_usuario.codigo_noticia IS
    'Código de noticia.';

COMMENT ON COLUMN tbl_flips_x_usuario.codigo_usuario IS
    'Código de usuario';

COMMENT ON COLUMN tbl_flips_x_usuario.codigo_revista IS
    'Código de la revista en la que fue flipeada la noticia.';

COMMENT ON COLUMN tbl_flips_x_usuario.fecha IS
    'Fecha en la que se hizo el Flip.';

ALTER TABLE tbl_flips_x_usuario ADD CONSTRAINT tbl_flips_x_usuario_pk PRIMARY KEY ( codigo_usuario,codigo_noticia,codigo_revista );
...
CREATE TABLE tbl_noticias (
    codigo_noticia        INTEGER NOT NULL,
    codigo_usuario_prop   INTEGER NOT NULL,
    codigo_revista        INTEGER NOT NULL,
    codigo_categoria      INTEGER NOT NULL,
...
COMMENT ON COLUMN tbl_notificaciones.codigo_usuario_receptor IS
    'El código del usuario de la notificación respectiva.';
...
COMMENT ON COLUMN tbl_usuarios.contraseña IS
...
CREATE TABLE tlb_seguidores (
    codigo_usuario_seguidor   INTEGER NOT NULL,
    codigo_usuario_seguido    INTEGER NOT NULL
);

COMMENT ON COLUMN tlb_seguidores.codigo_usuario_seguidor IS
    'Código del usuario que sigue.';

COMMENT ON COLUMN tlb_seguidores.codigo_usuario_seguido IS
    'Código del usuario que es seguido.';

ALTER TABLE tlb_seguidores ADD CONSTRAINT tlb_seguidores_pk PRIMARY KEY ( codigo_usuario_seguidor,codigo_usuario_seguido );
...
ALTER TABLE tbl_flips_x_usuario ADD CONSTRAINT fli_x_usu_rev_fk FOREIGN KEY ( codigo_revista )
    REFERENCES tbl_revistas ( codigo_revista );
...
ALTER TABLE tbl_notificaciones ADD CONSTRAINT not_est_not_fk FOREIGN KEY ( codgio_estado_notificacion )
    REFERENCES tbl_estado_notificacion ( codgio_estado_notificacion );
...
ALTER TABLE tbl_noticias ADD CONSTRAINT tbl_noti_tbl_usua_fk FOREIGN KEY ( codigo_usuario_prop )
    REFERENCES tbl_usuarios ( codigo_usuario );

ALTER TABLE tbl_flips_x_usuario ADD CONSTRAINT tbl_noticias_fk FOREIGN KEY ( codigo_noticia )
    REFERENCES tbl_noticias ( codigo_noticia );
...
ALTER TABLE tlb_seguidores ADD CONSTRAINT tbl_usuarios_fkv2 FOREIGN KEY ( codigo_usuario_seguidor )
    REFERENCES tbl_usuarios ( codigo_usuario );

ALTER TABLE tlb_seguidores ADD CONSTRAINT tbl_usuarios_fkv3 FOREIGN KEY ( codigo_usuario_seguido )
    REFERENCES tbl_usuarios ( codigo_usuario );

ALTER TABLE tbl_flips_x_usuario ADD CONSTRAINT tbl_usuarios_fkv4 FOREIGN KEY ( codigo_usuario )
    REFERENCES tbl_usuarios ( codigo_usuario );
...
CREATE SEQUENCE tbl_estado_notificacion_codgio START WITH 1 NOCACHE ORDER;

CREATE OR REPLACE TRIGGER tbl_estado_notificacion_codgio BEFORE
    INSERT ON tbl_estado_notificacion
    FOR EACH ROW
    WHEN (
        new.codgio_estado_notificacion IS NULL
    )
BEGIN
    :new.codgio_estado_notificacion := tbl_estado_notificacion_codgio.nextval;
END;
/
